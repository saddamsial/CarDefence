...
public class SmoothOrbitCam : MonoBehaviour
{
  ...
  public   float rotationXAxis = 0.0f;
    ...
    public bool panningOnXAxis = true;
    public bool panningOnYAxis = true;
    [HideInInspector]
	...
	public Vector3 panningWithLimits;
	public Vector3 defaulPanPosition;
    [HideInInspector]
    ...
    //enable / disable orbiting control
    public bool canControl = true;

    //enable / disable smooth position change
    public bool useSmoothPositionChange = false;
	...
	void Start()
	{
        ...
        defaulPanPosition = targetPanCam.transform.localPosition;
        ...
        velocityPanY = -(targetPanCam.transform.localPosition.y * 10 * (distance / 10));
    }
    ...
    public void ResetValues()
    {
        ...
        // rotationYTween?.Kill();
        // velocityTween?.Kill();
        // velocityPanYTween?.Kill();
        // velocityPanXTween?.Kill();
        //
        // rotationXTween = DOTween.To(()=> rotationXAxis, x=> rotationXAxis = x, angles.x, 0.5f);
        // rotationYTween = DOTween.To(()=> rotationYAxis, x=> rotationYAxis = x, angles.y, 0.5f);
        // velocityTween = DOTween.To(()=> velocityX, x=> velocityX =velocityY= x, 0, 0.5f);
        // velocityPanYTween= DOTween.To(()=> velocityPanY, x=>  velocityPanY= x, -(tempPanPosition.y / (distance / 10)), 0.5f);
        // velocityPanXTween= DOTween.To(()=> velocityPanX, x=> velocityPanX = x, -(tempPanPosition.x / (distance / 10)), 0.5f);
	...
	void LateUpdate()
	{
		...
		if (target)
		{
            ...
            if (target != null)
            {
                ...
                if (!useSmoothPositionChange)
                    transform.position = offsetPosition;
                else
                    transform.position = Vector3.Lerp(transform.position, offsetPosition, smoothTime * Time.deltaTime);
            }
            ...
            //panning
            tempPanPosition = new Vector3(-velocityPanX * distance / 10,
                -velocityPanY * distance / 10, 0);

            //apply panning
            if (targetPanCam != null && useable && enablePanning)
            {
                //panningWithLimits = new Vector3(panningOnXAxis ? tempPanPosition.x : targetPanCam.transform.localPosition.x,
                //    panningOnYAxis ? tempPanPosition.y : targetPanCam.transform.localPosition.y, 0);
                ...
                if (LimitPan) 
                {
                    ...
                    targetPanCam.transform.localPosition = new Vector3(clampX,
                        clampY, targetPanCam.transform.localPosition.z);
                }
            }
		}
